...
if (isset($_POST['submit'])) {
    ...
    $query = "INSERT INTO barcodebrg (id_brg, barcode_brg, qty, satuan, user) VALUES ";
    for ($i = 2; $i <= $baris; $i++) {
        ...
        $id_brg = $data->val($i, 1);
        $barcode_brg    = $data->val($i, 2);
        $qty    = $data->val($i, 3);
        $satuan    = $data->val($i, 4);
        $user    = $data->val($i, 5);

        //      setelah data dibaca, masukkan ke tabel pegawai sql
        $query .= "($id_brg, '$barcode_brg',$qty, '$satuan', '$user'),";
