...
$start = $time;
?>
<form name="myForm" id="myForm" onSubmit="return validateForm()" action="importKeluar.php" method="post" enctype="multipart/form-data">
...
//       membaca data (kolom ke-1 sd terakhir)
        $id_det_klr = $data->val($i, 1);
        $id_klr     = $data->val($i, 2);
        $id         = $data->val($i, 3);
        $jml_klr    = $data->val($i, 4);
        $jam        = $data->val($i, 5);

//      setelah data dibaca, masukkan ke tabel pegawai sql

        $insert_brg = $koneksi->query("INSERT INTO detail_keluar (id_det_klr, id_klr, id, jml_klr, jam)
                                    VALUES    ('$id_det_klr', '$id_klr', '$id', '$jml_klr', '$jam')");
