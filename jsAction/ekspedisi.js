$(document).ready(function(){
	...
	$("#activeEkspedisi").addClass('active');

	tabelEkspedisi = $('#tabelEkspedisi').DataTable({
		'ajax' : 'action/ekspedisi/fetchekspedisi.php',
		'order':[]
	});//manage tabel 

	$('#submitAddEkspedisi').unbind('submit').bind('submit', function() {
		const nopol = $('#nopol').val().trim();
		const supir = $('#supir').val().trim();
		const jenis = $('#jenis').val().trim();

		validateInput(nopol, '#nopol', 'Plat Nomor harus diisi');
		validateInput(supir, '#supir', 'Supir harus diisi');
