...
$(document).ready(function() {
	...
	if (divRequest == 'plus') {
		...
		$('#activeKoreksiPlus').addClass('active');

		tabelPlus = $("#tabelPlus").DataTable({
			'ajax' : 'action/barang/fetchPlus.php',
			'order' : [],
		});

		$("#addPlusBtnModal").unbind('click').bind('click', function() {

			$("#submitPlus").unbind('submit').bind('submit', function() {

				var brg = $("#id_brgPlus").val();
				var rak = $("#id_rakPlus").val();
				var jml = $("#jmlPlus").val();
/*				var ket = $("#ketMinus").val();
				...
				if (brg == "") {
					$("#id_brgPlus").before('<span class="help-inline bawah">Nama Barang Masih Kosong</span>');
					$('#id_brgPlus').closest('.control-group').addClass('error');
				}
				else
				{
					$("#id_brgPlus").find('.help-inline').remove();
					$("span").remove(":contains('Nama Barang Masih Kosong')");
				}

				if (rak == "") {
					$("#id_rakPlus").before('<span class="help-inline bawah">Nama Barang Masih Kosong</span>');
					$('#id_rakPlus').closest('.control-group').addClass('error');
				}
				else
				{
					$("#id_rakPlus").find('.help-inline').remove();
					$("span").remove(":contains('Nama Barang Masih Kosong')");
				}

				if (jml == "") {
					$("#jmlPlus").before('<span class="help-inline bawah">Jumlah Koreksi Masih Kosong</span>');
					$('#jmlPlus').closest('.control-group').addClass('error');
				}
				else
				{
					$("#jmlPlus").find('.help-inline').remove();
				...
				if (brg && rak && jml) {
					...
					$.ajax({
						...
						success:function(response)
						{
							...
							$("#simpanPlusBtn").button('reset');

							if (response.success == true) {

								$("#submitPlus")[0].reset();
								tabelPlus.ajax.reload(null, false);
								...
								//tampil pesan true
								$('#pesanPlus').html('<div class="alert alert-success">'+
								...
								$(".control-group").removeClass('error').removeClass('success');
								//tampil pesan false
								$('#pesanPlus').html('<div class="alert alert-error">'+
								'<button class="close" data-dismiss="alert">×</button>'+
								response.messages+'</div>');
							}
