<?php
	...
	$sql = "SELECT no_faktur, rak, brg, tgl, jam, jml_klr, id_det_klr, ket, MONTH(tgl) AS bulan, YEAR(tgl) AS tahun
			FROM keluar
	...
	if ($bulan == $row['bulan'] AND $tahun == $row['tahun']) {
		...
		$edit = '<li><a href="#editModalMinus" id="editMinus" onclick="editMinus('.$id_det_klr.')" data-toggle="modal"><i class="icon-pencil"></i> Edit</a></li>';

		$hapus = '<li><a href="#hapusModalKoreksi" onclick="hapusMinus('.$id_det_klr.')" data-toggle="modal"><i class="icon-trash"></i> Hapus</a></li>';
