...
try {
    ...
    $result = handleUpdateTahunSO($detailSaldoClass, $saldoClass, $soClass, $inputs, $conn);
...
function handleUpdateTahunSO($detailSaldoClass, $saldoClass, $soClass, $inputs, $conn)
{
    global $valid;
    ...
    //check tahun produksi ada atau tidak
    $checkDetailSaldoByIdYear = $detailSaldoClass->getDetailSaldoByidAndYearProd($id_detailsaldo, $inputs['tahunprod']);
    $resultCheckDetailSaldoByIdYear = $checkDetailSaldoByIdYear->fetch_assoc();
    $id_detailsaldoLatest = $resultCheckDetailSaldoByIdYear['id'];
...
function getInputs($koneksi)
{
    ...
    $requiredFields = [
        "tahunid_pro" => "ID Produk",
        "tahunprod" => "Tahun Produksi"
    ];
    ...
    if (!is_numeric($inputs['tahunprod']) || $inputs['tahunprod'] <= 0) {
        $valid['success'] = false;
        $valid['messages'] = "<strong>Error! </strong> Tahun Produksi Harus Berupa Angka Positif.";
        return $valid;
    }

    return [
        "id_pro" => $inputs['tahunid_pro'],
        "tahunprod" => $inputs['tahunprod']
    ];
}
