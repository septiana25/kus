...
function handlePrintPromosiMasuk($promosiClass, $tanggal_awal, $tanggal_akhir)
{
    $result = $promosiClass->getDataDetailPromosiMasuk($tanggal_awal, $tanggal_akhir);
...
function exportToExcel($data)
{
    ...
    foreach ($data as $item) {
        $sheet->setCellValue('A' . $row, $item['no_transaksi']);
        ...
        $sheet->setCellValue('G' . $row, date('d-m-Y', strtotime($item['tanggal'])));
        $totalQty += $item['qty'];
        $row++;
    }
...
try {
    ...
    $result = handlePrintPromosiMasuk($promosiClass, $tanggal_awal, $tanggal_akhir);
