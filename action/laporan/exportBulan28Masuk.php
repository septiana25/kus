...
$excelku->getActiveSheet()->mergeCells('A1:AI1');
$excelku->getActiveSheet()->mergeCells('A2:AI2');
...
$SI->setCellValue('AH3', 'T.MASUK'); //Kolom
$SI->setCellValue('AI3', 'S.AKHIR'); //Kolom
//Mengeset Syle nya
...
$excelku->getActiveSheet()->setSharedStyle($headerStylenya, "A3:AI3");
...
$strsql = "SELECT s.brg, s_awal, IFNULL(total_keluar, NULL) AS total_keluar,
            ...
            IFNULL(total_masuk, NULL) AS b_masuk, s_akhir, kat
          FROM(
...
while ($row = $res->fetch_assoc()) {
  ...
  $SI->setCellValue("AH".$baris,$row['b_masuk']); //mengisi data untuk TELP
  $SI->setCellValue("AI".$baris,$row['s_akhir']); //mengisi data untuk TELP
...
$SI->setCellValue("AH".$baris,$b_masuk); //mengisi data untuk TELP
