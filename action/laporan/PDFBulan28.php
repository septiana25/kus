<?php
$query = "SELECT s.brg, s_awal, IFNULL(total_masuk, 0) AS b_masuk,
  ...
  IFNULL(total_keluar, 0) AS total_keluar, s_akhir
FROM(
	...
	function Mybody($file, $type, $datas){
		if ($type=='file') {
			...
			while ($row = $datas->fetch_array()) {
				$this->Cell(14,5,$no,1,0,'C');
				$this->Cell(75,5,utf8_encode($row['brg']),1,0,'L');
				...
				$this->Cell(14,5,$row[32],1,0,'C');
				$this->Cell(14,5,$row[33],1,0,'C');
				$this->Ln();
				$no++;
			}
