...
$excelku->getActiveSheet()->getColumnDimension('AL')->setWidth(20);

// Mergecell, menyatukan beberapa kolom
$excelku->getActiveSheet()->mergeCells('A1:AL1');
$excelku->getActiveSheet()->mergeCells('A2:AL2');
...
$SI->setCellValue('AI3', 'T.ADJUSMEN'); //Kolom
$SI->setCellValue('AJ3', 'T.KELUAR'); //Kolom
$SI->setCellValue('AK3', 'S.AKHIR'); //Kolom
$SI->setCellValue('AL3', 'KODE'); //Kolom
//Mengeset Syle nya
...
$strsql = "SELECT s.brg, s_awal, IFNULL(total_masuk, NULL) AS b_masuk,
          ...
          adjusmen, tgl_29, IFNULL(total_keluar, NULL) AS total_keluar, s_akhir, kat, s.kdbrg
        FROM(
...
while ($row = $res->fetch_assoc()) {
  ...
  $SI->setCellValue("AI".$baris,$row['adjusmen']); //mengisi data untuk TELP
  $SI->setCellValue("AJ".$baris,$row['total_keluar']); //mengisi data untuk TELP
  $SI->setCellValue("AK".$baris,$row['s_akhir']); //mengisi data untuk TELP
  $SI->setCellValue("AL".$baris,$row['kdbrg']); //mengisi data untuk TELP
...
$SI->setCellValue("AI".$baris,$adj); //mengisi data untuk TELP
$SI->setCellValue("AJ".$baris,$t_akhir); //mengisi data untuk TELP
$SI->setCellValue("AK".$baris,$s_akhir); //mengisi data untuk TELP

//Membuat garis di body tabel (isi data)
$excelku->getActiveSheet()->setSharedStyle($bodyStylenya, "A5:AL$baris");
