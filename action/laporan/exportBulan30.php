...
$excelku->getActiveSheet()->getColumnDimension('AM')->setWidth(20);

// Mergecell, menyatukan beberapa kolom
$excelku->getActiveSheet()->mergeCells('A1:AM1');
$excelku->getActiveSheet()->mergeCells('A2:AM2');
$excelku->getActiveSheet()->mergeCells('F3:AI3');
...
$SI->setCellValue('AJ3', 'T.ADJUSMEN'); //Kolom
$SI->setCellValue('AK3', 'T.KELUAR'); //Kolom
$SI->setCellValue('AL3', 'S.AKHIR'); //Kolom
$SI->setCellValue('AM3', 'KODE'); //Kolom
//Mengeset Syle nya
...
$excelku->getActiveSheet()->setSharedStyle($headerStylenya, "A3:AM3");
$excelku->getActiveSheet()->setSharedStyle($headerStylenya, "A4:AM4");
...
$strsql = "SELECT s.brg, s_awal, IFNULL(total_masuk, NULL) AS b_masuk,
          ...
          tgl_29, tgl_30, adjusmen, IFNULL(total_keluar, NULL) AS total_keluar, s_akhir, kat, s.kdbrg
          FROM(
...
while ($row = $res->fetch_assoc()) {
  ...
  $SI->setCellValue("AJ".$baris,$row['adjusmen']); //mengisi data untuk TELP
  $SI->setCellValue("AK".$baris,$row['total_keluar']); //mengisi data untuk TELP
  $SI->setCellValue("AL".$baris,$row['s_akhir']); //mengisi data untuk TELP
  $SI->setCellValue("AM".$baris,$row['kdbrg']); //mengisi data untuk TELP
...
$SI->setCellValue("AJ".$baris,$adj); //mengisi data untuk TELP
$SI->setCellValue("AK".$baris,$s_akhir); //mengisi data untuk TELP
$SI->setCellValue("AL".$baris,$s_akhir); //mengisi data untuk TELP

//Membuat garis di body tabel (isi data)
$excelku->getActiveSheet()->setSharedStyle($bodyStylenya, "A5:AM$baris");
